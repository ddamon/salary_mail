...
class CTKLoginWindow(ctk.CTk):
    ...
    def create_login_form(self, parent):
        ...
        self.username_entry.bind('<Return>', self.on_username_return)
        self.password_entry.bind('<Return>', self.on_password_return)
    ...
    def on_username_return(self, event):
        """处理用户名输入框回车键事件"""
        try:
    ...
    def on_password_return(self, event):
        """处理密码输入框回车键事件"""
        try:
            self.login()
        except:
...
class CTKMessageDialog(ctk.CTkToplevel):
    ...
    def safe_focus_button(self, button):
        """安全地设置按钮焦点"""
        try:
            if self.winfo_exists() and button:
                button.focus()
        except:
            pass
